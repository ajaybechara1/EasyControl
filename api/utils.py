...
def train_user_model(user_token):
    try:
        ...
        logger.info("train_user_model token : %s", str(user_token), extra={'AppName': 'API'})
        ...
        logger.error("train_user_model %s at line %s", str(exc_tb.tb_lineno), str(e), extra={'AppName': 'API'})
...
def rgb_to_grey_scale_source_to_target(source_path, target_path):
    try:
        ...
        # logger.info("rgb_to_grey_scale_source_to_target source_path : %s , target_path : %s", str(source_path), str(target_path), extra={'AppName': 'API'})
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        logger.error("rgb_to_grey_scale_source_to_target %s at line %s", str(exc_tb.tb_lineno), str(e), extra={'AppName': 'API'})
...
def rgb_to_grey_scale_for_given_token(user_token):
    try:
        ...
        logger.info("rgb_to_grey_scale_for_given_token token : %s", str(user_token), extra={'AppName': 'API'})
        ...
        return "GREY SCALE CONVERT SUCCESS"
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        logger.error("rgb_to_grey_scale_for_given_token %s at line %s", str(exc_tb.tb_lineno), str(e), extra={'AppName': 'API'})
        return "GREY SCALE CONVERT ERROR"

# rgb_to_grey_scale_for_given_token("ABCDEF")

def train_model_with_token(user_token):
    print("AJAY")
    logger.info("train_model_with_token token : %s", str(user_token), extra={'AppName': 'API'})

    grey_scale_resule = rgb_to_grey_scale_for_given_token(user_token)
    training_result = train_user_model(user_token)

    logger.info("training done token : %s", str(user_token), extra={'AppName': 'API'})
